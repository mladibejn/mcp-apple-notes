...
export class PerformanceProfiler {
    ...
    endOperation(operationName: string, metadata?: Record<string, any>): void {
        ...
        if (!metric) {
            this.logger.warn(`No active operation found for: ${operationName}`);
            return;
        }
        ...
        this.logger.debug(`Operation completed: ${operationName} (duration: ${metric.duration.toFixed(2)}ms)`);
    }
    ...
    async runBenchmark(
        ...
        try {
            ...
            for (let i = 0; i < iterations; i += concurrentOperations) {
                ...
                const batchPromises = Array(batch).fill(0).map(async () => {
    ...
    compareWithBaseline(operationName: string): Record<string, number> {
        ...
        return {
            durationDiff: ((current.duration || 0) - (baseline.duration || 0)) / (baseline.duration || 1) * 100,
            throughputDiff: ((current.throughput || 0) - (baseline.throughput || 0)) / (baseline.throughput || 1) * 100,
            memoryUsageDiff: ((current.memory?.heapUsed || 0) - (baseline.memory?.heapUsed || 0)) / (baseline.memory?.heapUsed || 1) * 100,
        };
    }
    ...
    static profileMethod(operationName?: string) {
        return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
            ...
            const profiler = new PerformanceProfiler(target.logger || console);
            ...
            descriptor.value = async function (...args: any[]) {
                ...
                try {
                    ...
                    profiler.endOperation(methodName, { error: error instanceof Error ? error.message : 'Unknown error' });
                    throw error;
                }
            };

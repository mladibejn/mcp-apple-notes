 ...
 */
export async function writeJSON<T>(data: T, filePath: string, pretty = true): Promise<void> {
  try {
    ...
    if (error instanceof Error) {
      throw new JSONFileError(
        `Error writing JSON to file ${JSON.stringify(filePath)}: ${error.message}`,
        error
      );
    }
...
export async function updateJSON<T>(
  ...
  try {
    ...
    if (error instanceof Error) {
      throw new JSONFileError(`Error updating JSON file ${filePath}: ${error.message}`, error);
    }
...
export async function writeJSONSafe<T>(data: T, filePath: string): Promise<void> {
  ...
  try {
    ...
    if (error instanceof Error) {
      throw new JSONFileError(`Error in safe JSON write to ${filePath}: ${error.message}`, error);
    }

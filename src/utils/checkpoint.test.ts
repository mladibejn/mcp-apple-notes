...
describe('CheckpointManager', () => {
  ...
  test('should track stage progress correctly', async () => {
    ...
    expect(checkpointManager.getStageProgress(ProcessingStage.RAW_EXPORT).status).toBe(
    ...
    expect(checkpointManager.getStageProgressPercentage(ProcessingStage.RAW_EXPORT)).toBe(
      (2 / 3) * 100
    );
  });
  ...
  test('should handle stage completion', async () => {
    ...
    expect(
      checkpointManager.getStageProgress(ProcessingStage.RAW_EXPORT).completionTime
    ).toBeDefined();
  });
  ...
  test('should handle file operations correctly', async () => {
    ...
    expect(newManager.getStageProgress(ProcessingStage.RAW_EXPORT).status).toBe(
      StageStatus.IN_PROGRESS
    );
  });
});

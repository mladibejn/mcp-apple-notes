        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a note summarization assistant. Create a concise summary of the following note content in 2-3 sentences.',
          },
          {
            role: 'user',
            content,
        ...
        messages: [
          {
            role: 'system',
            content:
              'Extract 3-5 relevant tags from the following note content. Return only the tags as a comma-separated list, without any additional text.',
          },
          {
            role: 'user',
            content,
  ...
  async generateSummaryAndTags(content: string): Promise<{ summary: string; tags: string[] }> {
    ...
    return this.retryWithExponentialBackoff(async () => {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are helping organize a collection of personal notes. For the input text, provide a 1-2 sentence summary and extract 3-5 relevant tags. Respond in JSON format: {"summary": "<SUMMARY>", "tags": "<TAG1, TAG2, TAG3>"}',
          },
          {
            role: 'user',
            content,
      ...
      const result = JSON.parse(
        response.choices[0]?.message?.content || '{"summary": "", "tags": ""}'
      );
      return {
